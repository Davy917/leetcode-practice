...
class Solution:
    def maxSatisfied(self, customers: List[int], grumpy: List[int], minutes: int) -> int:
        ...
        for right in range(minutes, len(customers)):
            add_satisfied = sum(customers[i] for i in range(left, right+1) if grumpy[i] == 1)#left是閉區間, right+1是開區間
    ...
    def maxSatisfied_v2(self, customers: List[int], grumpy: List[int], minutes: int) -> int:
        base_satisfied = sum(customers[i] for i in range(len(customers)) if grumpy[i] == 0)
        add_satisfied = sum(customers[i] for i in range(minutes) if grumpy[i] == 1)
        max_add_satisfied = add_satisfied
...
if __name__ == "__main__":
    ...
    minutes = 3
    print("Ans = ", Solution().maxSatisfied_v2(customers, grumpy, minutes))
...
maxSatisfied_v2 是時間複雜度優化版, 在for迴圈裡 add_satisfied 不用每次都從頭算一次
