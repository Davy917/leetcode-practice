...
| 輪次 | 內層 while：left 移到哪 | 動作 | 交換後陣列 | right |
|------|------------------------|------|-----------|-------|
| 1 | arr[1]=2 ≤ 4 → left=2；arr[2]=7 > 4，停 | left(2)≠right(6)：exchange(2,6)，right-- | `[4, 2, 5, 1, 6, 3, 7]` | 5 |
| 2 | arr[2]=5 > 4，立即停（left 不動） | left(2)≠right(5)：exchange(2,5)，right-- | `[4, 2, 3, 1, 6, 5, 7]` | 4 |
| 3 | arr[2]=3 ≤ 4 → left=3；arr[3]=1 ≤ 4 → left=4；left<right 不成立，退出 | left(4)==right(4)：不做交換 | `[4, 2, 3, 1, 6, 5, 7]` | 4 |

**收尾**：
- left==right==4，arr[4]=6 > 4 → **right--** → right=3
- right(3) ≠ start(0) → exchange(arr, 0, 3) → `[1, 2, 3, 4, 6, 5, 7]`
- **middle = 3**（arr[3] = 4）✓
