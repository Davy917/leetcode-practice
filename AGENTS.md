...
- IntelliJ module source roots are tracked in `leetcode-solutions.iml`; when adding a new Java problem folder, mark it as Source Root (or update `.iml`).
...
- If you add a new Java problem folder, ensure IDE discoverability via Source Root settings (`leetcode-solutions.iml`) per existing workflow in `README.md`.
